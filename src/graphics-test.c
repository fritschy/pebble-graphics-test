...
static inline rgb_t divrgb(rgb_t a, int d) {
   return (rgb_t) { a.r / d, a.g / d, a.b / d };
}
...
static void /*__attribute__((optimize(2)))*/ fbFillGradient(GPoint p0, GPoint p1, uint8_t c0_, uint8_t c1_) {
   rgb_t c0 = to_rgb(c0_);
   rgb_t c1 = to_rgb(c1_);
...
#else
      for (int x = 0; x < FBW; x++) {
         rgb_t oldp = addrgb(c, addrgb(next, error[0][x]));
         error[0][x] = (rgb_t) { 0, 0, 0 };          // set to zero for next row

         rgb_t newp = palette(oldp);
         fb[y*FBW+x] = from_rgb(clamprgb(newp));
         rgb_t quante = subrgb(oldp, newp);

         // using sierra-lite error diffusion coefficients which are very computation friendly
         //
         // [ _ X 2 ]
         // [ 1 1 _ ]
         //    1/4

         quante = divrgb(quante, 4);
         next = addrgb(quante, quante);
         error[1][x - 1] = addrgb(error[1][x - 1], quante);  // accumulate
         error[1][x    ] = quante;                           // overwrite
      }
#endif
...
static void draw(void) {
   ...
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xf0, 0xff); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xf0, 0xc3); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xf0, 0xff); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xcc, 0xcf); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xfc, 0xcf); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xf0, 0xff); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xf3, 0xcf); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xf0, 0xff); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xcc, 0xfc); // red -> white
   fbFillGradient(GPoint(0,0), GPoint(FBW, FBH), 0xf0, 0xff); // red -> white
